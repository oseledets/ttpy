...
## 3b. KSL: почему сравнение по времени с легаси не имеет смысла

Задача: `d = 6`, `n = 2`, случайный симметричный `A` с `||A||_2 = 1`, `y0`
ранга 4, одинаковые ядра из файла для обеих реализаций, b300/numpy/float64,
минимум из 5 прогонов.

| | время | относительная ошибка против плотного `expm` |
|---|---|---|
| легаси (Fortran), τ = 0.05 | **0.25 мс** | 1.101 |
| легаси, τ = 0.20 | 0.24 мс | 1.107 |
| легаси, τ = 0.80 | 0.25 мс | 1.148 |
| ttpy2, τ = 0.05 | 4.30 мс | 9.12e-03 |
| ttpy2, τ = 0.20 | 4.28 мс | 3.14e-02 |
| ttpy2, τ = 0.80 | 4.34 мс | 9.32e-02 |

Легаси в 17 раз быстрее и при этом не решает задачу. Проверка, не зависящая ни
от плотного эталона, ни от соглашений об индексах: **при τ → 0 интегратор обязан
вернуть `y0`**, каким бы ни был `A`.

| τ | легаси `‖y − y0‖/‖y0‖` | ttpy2 |
|---|---|---|
| 1e-8 | **1.100** | 3.37e-09 |
| 1e-4 | **1.100** | 3.37e-05 |
| 0.05 | 1.100 | 1.72e-02 |

У легаси ошибка не зависит от `τ` вовсе, а норма результата 1.0827 вместо 1; он
же печатает «Solving a complex-valued dynamical problem» на вещественном входе,
то есть уходит в комплексную ветку — ту самую, где найден дефект порядка K/S
(см. `COMPAT.md`). У нас ошибка ровно пропорциональна `τ`, как и должно быть.

Отсюда правило для этой таблицы: **время сравнивается только при сопоставимой
корректности.** Наши 4.3 мс — честная цена 22 локальных экспонент
(`2(2d−1)`) с Крыловским подшагованием; это не быстро для такой мелкой задачи и
остаётся целью оптимизации, но сравнивать её с 0.25 мс неверного ответа нельзя.

Отдельно измерена цена сторожа жёсткости, добавленного в `_step_exp` (две нормы
на локальную экспоненту): **4.33 → 4.45 мс, +2.8 %**.
