...
| `amen_solve` d=12, eps=1e-6 | **16.3 мс** | 488 мс | **30x медленнее** |
...
* **`amen_solve` медленнее в 30 раз** при одинаковой точности (невязка 1.8e-7
  против 2.0e-7) и сопоставимых рангах. Это самое слабое место пакета, и оно
  измерено, а не оценено: тысячи мелких локальных решений в интерпретаторе.
  Уже сделано: канонизация dtype вызывалась 80 тыс. раз за один решатель и
  съедала 30% времени (ускорена в 49 раз). Осталось: батчевать локальные
  системы и уводить их на GPU.
